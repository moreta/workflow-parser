...
function countNewlines(str, start, end) {
  let ret = 0;
  for (let i = start; i < end; i++) if (str.charAt(i) === "\n") ret++;
  return ret;
}
...
function lex(str) {
  ...
  while (str !== "") {
    ...
    // Strings, with escape characters
      ...
      let val = "";
      let warnedAboutIllegalChar = false;
        ...
        if (str.charAt(i) === "\\") {
          switch (str.charAt(++i)) {
            ...
            default:
              ret.push([
                "ERROR",
                `illegal escape sequence: "${str.charAt(i)}"`,
                linenum
              ]);
          }
          ...
          if (!warnedAboutIllegalChar) {
            ret.push([
              "ERROR",
              `control character in string: '\\u00${str
                .charCodeAt(i)
                .toString(16)
                .padStart(2, "0")}'`,
              linenum
    ...
    // Heredocs
      ...
      const endpos = str.indexOf(terminator, match[0].length);
      if (endpos === -1) {
        ret.push([
          "ERROR",
          `missing heredoc terminator: "${terminator}"`,
          linenum
        ]);
        return ret;
      }
